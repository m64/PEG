...
class DynamicObject (object):
    """A base class that only supports dynamic attributes functionality"""
    def __init__ (self, name="Dynamic object", **kwargs):
...
class GameObject (DynamicObject):
    ...
    def __init__ (self, ID, gfx = {}, xpos = 0.0, ypos = 0.0, map_id = None, 
        ...
        """Set the basic values that are shared by all game objects.
           ...
           """
        DynamicObject.__init__(self, name, **kwargs)
        self.ID = ID
        self.gfx = gfx
        self.X = xpos
        self.Y = ypos
...
class Lockable (Openable):
    ...
    def __init__ (self, locked = False, is_open = True, **kwargs):
        ...
        if locked == True: #HACK: For some reason locked appears to NOT be a bool. Needs investigation
            locked = True
        else: locked = False
        self.locked = locked
        if locked :
...
class Container (object):
    ...
    def __init__ (self,
        ...
        self.items = {}
        self.events = events
        self.containergui = None
        if not self.events:
            self.events = {'takeAllButton':self.hideContainer,
    ...
    def placeItem (self, item, index=None):
        ...
        if index == None:
            self.placeAtVacant(item)
        else:
            if index in self.items :
                raise ValueError('Slot %d is busy in %s' % (index, self.name))
    ...
    def hideContainer(self):
        if self.containergui:
            self.containergui.hideContainer()
    ...
    def findItem(self, **kwargs):
        """Find an item in container by various attributes. All parameters are optional.
           @type name: String
           @param name: Unique or non-unique object name. If the name is non-unique, first matching object is returned
           @type kind: String
           @param kind: One of the possible object kinds like "openable" or "weapon"
           @return: The item matching criteria or None if none was found
        """
        for i in self.items :
            if "name" in kwargs and self.items[i].name != kwargs["name"]:
                continue
            if "kind" in kwargs and not self.items[i].trueAttr(kwargs["kind"]):
                continue
            return self.items[i]
        return None
