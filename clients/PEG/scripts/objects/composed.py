...
class CarryableContainer(DynamicObject, Container, Carryable):
    ...
    def __repr__(self):
        return "[%s"%self.name +str(reduce((lambda a,b: a +', '+str(b)), self.items,""))+" ]"
...
class CarryableItem (GameObject, Carryable):
    """Composite class that will be used for all carryable items, weapons, etc.
    """
    def __init__(self, **kwargs):
        GameObject.__init__(self,**kwargs)
        Carryable.__init__(self,**kwargs)
...
class Door(GameObject, Lockable, Scriptable, Trappable):
    ...
    def __init__ (self, target_map_name = 'my-map', target_map = 'map/map.xml', target_x = 0.0, \
                        target_y = 0.0, **kwargs):
        GameObject   .__init__(self, **kwargs)
        Lockable     .__init__(self, **kwargs)
        Scriptable   .__init__(self, **kwargs)
